...
function LoopChecker(sketch, maxRunTime) {
  var self = {};
  var startTime = Date.now();
  var loopCheckFailureRange = null;

  window.__loopCheck = function(range) {
...
function startSketch(sketch, p5version, maxRunTime, errorCb) {
  var sketchScript = document.createElement('script');
  var loopChecker = LoopChecker(sketch, maxRunTime);
